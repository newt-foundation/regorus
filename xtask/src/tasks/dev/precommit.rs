...
impl PrecommitCommand {
    pub fn run(&self) -> Result<()> {
        ...
        run_cargo_step(&workspace, "cargo build --all-targets", ["build", "--all-targets"])
            .with_context(|| "pre-commit: cargo build --all-targets failed".to_string())?;

        run_cargo_step(&workspace, "cargo xtask fmt --check", ["xtask", "fmt", "--check"])
...
fn verify_clean_status(root: &std::path::Path) -> Result<()> {
    ...
    if !output.status.success() {
        return Err(anyhow!("git status -s exited with status {}", output.status));
    }

    let stdout = String::from_utf8(output.stdout).with_context(|| "git status output was not UTF-8".to_string())?;
