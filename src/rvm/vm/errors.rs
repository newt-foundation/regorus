...
pub enum VmError {
    #[error(
        "Execution stopped: exceeded maximum instruction limit of {limit} after {executed} instructions (pc={pc})"
    )]
    ...
    RegisterNotArray { register: u8, value: Value, pc: usize },

    #[error("Register {register} does not contain a set (value={value:?}, pc={pc})")]
    RegisterNotSet { register: u8, value: Value, pc: usize },
    ...
    BuiltinArgumentMismatch { expected: u16, actual: usize, pc: usize },
    ...
    InvalidAddition { left: Value, right: Value, pc: usize },

    #[error("Cannot subtract {left:?} and {right:?} (pc={pc})")]
    InvalidSubtraction { left: Value, right: Value, pc: usize },

    #[error("Cannot multiply {left:?} and {right:?} (pc={pc})")]
    InvalidMultiplication { left: Value, right: Value, pc: usize },

    #[error("Cannot divide {left:?} and {right:?} (pc={pc})")]
    InvalidDivision { left: Value, right: Value, pc: usize },

    #[error("modulo on floating-point number (left={left:?}, right={right:?}, pc={pc})")]
    ModuloOnFloat { left: Value, right: Value, pc: usize },

    #[error("Cannot modulo {left:?} and {right:?} (pc={pc})")]
    InvalidModulo { left: Value, right: Value, pc: usize },

    #[error("Cannot iterate over {value:?} (pc={pc})")]
    InvalidIteration { value: Value, pc: usize },

    #[error(
        "HostAwait executed but no response provided for destination register {dest} (id: {identifier:?}, pc={pc})"
    )]
    HostAwaitResponseMissing { dest: u8, identifier: Value, pc: usize },
    ...
    InvalidEntryPointIndex { index: usize, max_index: usize, pc: usize },
    ...
    InvalidFunctionCallParamsIndex { index: u16, pc: usize, available: usize },

    #[error("Invalid builtin call params index: {index} (pc={pc}, available={available})")]
    InvalidBuiltinCallParamsIndex { index: u16, pc: usize, available: usize },

    #[error("Invalid builtin info index: {index} (pc={pc}, available={available})")]
    InvalidBuiltinInfoIndex { index: u16, pc: usize, available: usize },
