...
pub fn map_binding_error(err: BindingPlannerError) -> Error {
    match err {
        BindingPlannerError::ColonEqualsRequiresBindableLeft { span } => {
            span.error("assignment operator := requires left-hand side to have bindable variables")
        }
        BindingPlannerError::ArraySizeMismatch { span, .. } | BindingPlannerError::ArrayLengthMismatch { span, .. } => {
            span.error("mismatch in number of array elements")
        }
        BindingPlannerError::ObjectLiteralKeysMismatch { expected, actual, span } => span.error(&format!(
        ...
        BindingPlannerError::ObjectKeyNotFound { key, span } => span.error(&format!(
...
impl BindingPlannerError {
    pub(crate) fn to_span_message(&self) -> String {
        match self {
            BindingPlannerError::ColonEqualsRequiresBindableLeft { span } => span.message(
                "error",
                "assignment operator := requires left-hand side to have bindable variables",
            ),
            BindingPlannerError::ArraySizeMismatch {
                ...
                let detail =
                    format!("mismatch in number of array elements (left has {left_size}, right has {right_size})");
                span.message("error", detail.as_str())
            }
            BindingPlannerError::ArrayLengthMismatch { expected, actual, span } => {
                let detail = format!("array length mismatch. Expected {expected} got {actual}.");
                span.message("error", detail.as_str())
            }
            BindingPlannerError::ObjectLiteralKeysMismatch { expected, actual, span } => {
                let detail = format!(
                    "object literal keys mismatch. Expected keys {:?} got {:?}.",
                    expected, actual
                );
            ...
            BindingPlannerError::ObjectKeyNotFound { key, span } => {
                let detail = format!("key \"{key}\" not found in left-hand side object during destructuring");
                span.message("error", detail.as_str())
            }
            BindingPlannerError::VariableAlreadyDefined { var, span } => {
            ...
            BindingPlannerError::IncompatibleDestructuringPatterns { span } => span.message(
                "error",
